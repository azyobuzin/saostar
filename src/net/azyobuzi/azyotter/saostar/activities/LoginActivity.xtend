...
class LoginActivity extends Activity {
	private val Handler h = new Handler()
	...
	override onCreate(Bundle savedInstanceState) {
        ...
        status = findViewById(R.id.tv_login_waiting_status) as TextView
        ...
        new Thread(new Runnable() {
			override run() {
				try {
					val tw = Twitter4JFactories.twitterFactory.getInstance()
	...
	override onNewIntent(Intent intent) {
		...
		if (intent.getData() != null) {
			val verifier = intent.getData().getQueryParameter("oauth_verifier")
			...
			if (!StringUtil.isNullOrEmpty(verifier)) {
				...
				new Thread(new Runnable() {
					override run() {
						try {
							val tw = Twitter4JFactories.twitterFactory.getInstance();
							val token = tw.getOAuthAccessToken(reqToken);

							if (canceled) return;

							if (Accounts.get(token.getUserId()) == null) {
								val a = new Account(token.getUserId())
